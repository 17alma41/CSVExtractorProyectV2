...
INPUT_FOLDER = os.path.abspath(os.path.join(BASE_DIR, "../data/inputs"))
CLEAN_INPUT_FOLDER = os.path.abspath(os.path.join(BASE_DIR, "../data/clean_inputs"))
OUTPUT_FOLDER = os.path.abspath(os.path.join(BASE_DIR, "../data/outputs"))
EXTRACTOR_FOLDER = os.path.abspath(os.path.join(BASE_DIR, "../extractor"))
TXT_CONFIG_DIR = os.path.abspath(os.path.join(BASE_DIR, "../config/txt_config"))
...
def procesar_sitio(row_dict):
    raw = row_dict.get("website", "")
    # 1) Saltar NaN (pandas) o valores no-string
    if pd.isna(raw) or not isinstance(raw, str):
        print(f"⚠️ URL inválida o ausente, saltando: {raw!r}")
        return {**row_dict, "email": "", "facebook": "", "instagram": "", "linkedin": "", "x": ""}

    website = raw.strip()
    # 2) Saltar si no empieza por http(s)
    if not website.lower().startswith(("http://", "https://")):
        print(f"⚠️ URL no HTTP, saltando: {website}")
        return {**row_dict, "email": "", "facebook": "", "instagram": "", "linkedin": "", "x": ""}
